...
<div class="row">
    <div class="col s12">
                                ...
                                <table id="list-datatable" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td> {{ $record->label }} </td>
                                            <td class="hide-on-small-only"> {{ $record->desc }} </td>
                                            <td>
                                                <a href="{{route('role_update', ['role'=>$record->id])}}"><i
        ...
        <div style="bottom: 50px; right: 19px;" class="fixed-action-btn direction-top"><a href="{{route('role_create')}}" class="btn-floating btn-large gradient-45deg-light-blue-cyan gradient-shadow"><i class="material-icons">add</i></a></div>
    </div>
</div>
...
<script>
    ...
    function suppRecord() {
        window.location.replace("/role/delete/"+$("#delId").val());
    }
