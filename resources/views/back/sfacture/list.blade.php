...
<div class="row">
    <div class="col s12">
                                ...
                                <table id="list-datatable" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td> {{ $record->date }} </td>
                                            <td> {{ $record->total_ht }} </td>
                                            <td> {{ $record->taux_tva }} </td>
                                            <td> {{ $record->total_tva }} </td>
                                            <td> {{ $record->total_ttc }} </td>
                                            <td>
                                                <a href="{{route('sfacture_update', ['sfacture'=>$record->id])}}"><i
        ...
        <div style="bottom: 50px; right: 19px;" class="fixed-action-btn direction-top"><a href="{{route('sfacture_create')}}" class="btn-floating btn-large gradient-45deg-light-blue-cyan gradient-shadow"><i class="material-icons">add</i></a></div>
    </div>
</div>
...
<script>
    ...
    function suppRecord() {
        window.location.replace("/sfacture/delete/"+$("#delId").val());
    }
