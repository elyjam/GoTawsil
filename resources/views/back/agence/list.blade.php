...
<div class="row">
    <div class="col s12">
                                ...
                                <table id="list-datatable" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td> {{ $record->code }} </td>
                                            <td> {{ $record->villeDetail->libelle }} </td>
                                            <td> {{ $record->adresse }} </td>
                                            <td> {{ $record->telephone }} </td>
                                            <td> {{ $record->email }} </td>
                                            <td> {{ $record->visible_site }} </td>
                                            <td> {{ $record->statut }} </td>
                                            <td> {{ $record->Libelle }} </td>
                                            <td>
                                                <a href="{{route('agence_update', ['agence'=>$record->id])}}"><i
        ...
        <div style="bottom: 50px; right: 19px;" class="fixed-action-btn direction-top"><a href="{{route('agence_create')}}" class="btn-floating btn-large gradient-45deg-light-blue-cyan gradient-shadow"><i class="material-icons">add</i></a></div>
    </div>
</div>
...
<script>
    ...
    function suppRecord() {
        window.location.replace("/agence/delete/"+$("#delId").val());
    }
