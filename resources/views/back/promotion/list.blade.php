                                ...
                                <table id="list-datatable" class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width : 100px" class="hide-on-small-only"> Du </th>
                                            <th style="width : 100px" class="hide-on-small-only"> Au </th>
                                            ...
                                            <td> {{ $record->libelle }}
                                                <p class="hide-on-med-and-up">
                                                    <span class="new badge gradient-45deg-light-blue-cyan"
                                                        data-badge-caption="{{ $record->type }}"></span>
                                                </p>
                                            </td>
                                            <td>
                                                @if($record->client == '0')
                                                <p style="color:white;background-color:#379d11;text-align:center">
                                                Tous les clients</p>
                                                @else
                                                <p style="color:white;background-color:#1991ce;text-align:center">{{ $record->clientDetail->libelle }}</p>
                                                @endif


                                            </td>
                                            <td class="hide-on-small-only"> {{ $record->description }} </td>
                                            <td class="hide-on-small-only"> {{ $record->date_debut }} </td>
                                            <td class="hide-on-small-only"> {{ $record->date_fin }} </td>
                                            <td>
                                                <a href="{{route('promotion_update', ['promotion'=>$record->id])}}"><i
...
function suppRecord() {
    window.location.replace("/promotion/delete/" + $("#delId").val());
}
