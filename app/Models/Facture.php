...
class Facture extends Model
{
    ...
    public static function getPaiementExpeditions($paiement)
    {
        ...
        $paiements = \DB::table("remboursements_expeditions")
            ->select('*', \DB::raw('agences_des.Libelle as destination'), \DB::raw('expeditions.created_at as created_at'))
            ->leftJoin('expeditions', 'remboursements_expeditions.expedition_id', '=', 'expeditions.id')
            ->leftJoin('villes as agences_des', 'agences_des.id', '=', 'expeditions.agence_des')
            ->where("remboursements_expeditions.remboursement_id", $paiement->remboursement)
            ->where("expeditions.client", $paiement->client)
            ->get();
        return $paiements;
    }
    ...
    public function printDetail($facture)
    {
        ...
        // Set some content to print
        $paiements = RemboursementPaiements::all()->where('facture', $facture->id);
        //dd($paiements);
        $html = '';
        foreach ($paiements as $paiement) {
            $html = '
            ...
            $expeditions = self::getPaiementExpeditions($paiement);
            $i = 0;
            $expeditionsTotal = $colisTotal = $fondsTotal = $fraisTotal = $netsTotal = 0;
            foreach ($expeditions as $expedition) {
                if ($i == 0) {
                    $html .= '
