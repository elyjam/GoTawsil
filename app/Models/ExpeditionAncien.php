...
class ExpeditionAncien extends Model
{
    protected $table = 'expeditions_ancien';
    protected $guarded = [];
    ...
    {
        return $this->belongsTo(\App\Models\Agence::class, 'origine');
    }
    public function DestinationDetail()
    {
        return $this->belongsTo(\App\Models\Agence::class, 'des');
    }
    public function agenceDesDetail()
    {
        return $this->belongsTo(\App\Models\Agence::class, 'agence_des');
    }
    ...
    {
        $expeditions = \DB::table("remboursements_expeditions_ancien")
            ->select(
                "*",
                \DB::raw('expeditions_ancien.id as id'),
                \DB::raw('expeditions_ancien.type as type'),
                \DB::raw('expeditions_ancien.num_expedition as num_expedition'),
                \DB::raw('expeditions_ancien.client as client_id'),
                \DB::raw('expeditions_ancien.fond as fond'),
                \DB::raw('expeditions_ancien.port as port'),
                \DB::raw('expeditions_ancien.telephone as telephone'),
                \DB::raw('expeditions_ancien.colis as colis'),
                \DB::raw('expeditions_ancien.retour_fond as retour_fond'),
                \DB::raw('expeditions_ancien.deleted as deleted'),
                \DB::raw('expeditions_ancien.deleted_at as deleted_at'),
                \DB::raw('expeditions_ancien.created_at as created_at'),
                \DB::raw('clients.libelle as client'),
            ...
            ->leftJoin('expeditions_ancien', 'expeditions_ancien.id', '=', 'remboursements_expeditions_ancien.expedition_id')
            ->leftJoin('clients', 'clients.id', '=', 'expeditions_ancien.client')
            ->leftJoin('villes as agences_des', 'agences_des.id', '=', 'expeditions_ancien.agence_des')
            ->leftJoin('villes as agences_exp', 'agences_exp.id', '=', 'expeditions_ancien.agence')
            ->orderBy('client_id', 'desc');
            $expeditions->where('expeditions_ancien.deleted', 0);
            $expeditions->where('remboursements_expeditions_ancien.remboursement_id', $remboursement);
        return  $expeditions->get();
    }
    ...
    public static function getExpeditionsByAllRemboursement()
    {
        $expeditions = \DB::table("remboursements_expeditions_ancien")
            ...
            ->leftJoin('expeditions', 'expeditions.id', '=', 'remboursements_expeditions_ancien.expedition_id')
            ->leftJoin('clients', 'clients.id', '=', 'expeditions.client')
            ->leftJoin('villes as agences_des', 'agences_des.id', '=', 'expeditions.agence_des')
            ->leftJoin('villes as agences_exp', 'agences_exp.id', '=', 'expeditions.agence')
            ->orderBy('client_id', 'desc');
        $expeditions->where('expeditions.deleted', 0);
        $expeditions->where('remboursements_expeditions_ancien.remboursement_id','!=' ,null);
        $expeditions->groupBy('remboursements_expeditions_ancien.remboursement_id');
    ...
    public static function getStockList($sens = null)
    {
        ...
        $expeditions = Expedition::whereIn('etape', ['2', '3', '10', '16','20']);
        if ($sens !== null) {
            $expeditions->where('sens', $sens);
        }
    ...
    public static function saveImportedList($expeditions, $villes, $numExp)
    {
        ...
        foreach ($expeditions as $expedition) {
            ...
            if ($numExp != 'AUTO') {
                ...
                $record->num_expedition =  'EX' . sprintf("%06d", Expedition::all()->count() + 1);
            }
